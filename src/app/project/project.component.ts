...
export class ProjectComponent implements OnInit {  
  ...
  formTitle = 'Add Project | Organization';
  btnSaveText = 'Save';
  ...
  ngOnInit() {
    this.entryForm = this.formBuilder.group({
      Id: [null],
      ProjectName: [null, [Validators.required, Validators.maxLength(250)]]
    });
    this.getList();
  }
  ...
  getItem(id) {
    ...
    this._service.get('api/mastersetting/GetProjectDetailById/' + id).subscribe(res => {
      ...
      this.entryForm.controls['ProjectName'].setValue(res.Record.ProjectName);
    }, err => {
      this.blockUI.stop();
      this.toastr.error(err.Message || err, 'Error!', { closeButton: true, disableTimeOut: true });
    });
  ...
  onFormSubmit() {
    ...
    const obj = {
      ...
      ProjectName: this.entryForm.value.ProjectName.trim()
    };
    
    const request = this._service.post('api/mastersetting/CreateOrUpdateProject', obj);
